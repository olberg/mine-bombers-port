...
| Offset | Size | Field | Evidence |
|--------|------|-------|----------|
| +0xD3 | 2 | start_pixel_x | Set by FUN_1010_c4f2 (seg_1010:7238). **NOTE: In the original, X = rows (horizontal), Y = columns (vertical).** |
| +0xD5 | 2 | start_pixel_y | Set by FUN_1010_c4f2 (seg_1010:7239) |
| +0xEE | 2 | pixel_x | `*(int*)(base + 0xEE)` used in move_player. **pixel_x = tile_col * 10 + 5** (FUN_1000_3b40:2530). tile_col = pixel_x / 10 |
...
### Byte-sized State Flags (offsets +0xD9 through +0xE6)

The unnamed data region shows many `undefined1` (byte) entries in this range, indicating boolean state flags:
- Animation state
- Direction facing
- Invulnerability timer
- Weapon-related flags
- Movement state
...
Per-player key fields (1 byte each):
```
+0x00: Left scancode
+0x01: Right scancode
+0x02: Up scancode
+0x03: Down scancode
+0x04: Stop scancode  (= base + 0x05 from key load order)
+0x05: Bomb/Buy scancode
+0x06: Choose/Sell scancode
+0x07: Extra scancode
```
...
The on-disk player database stores 32 records of 101 bytes each. This is a separate (smaller) record from the in-memory 266-byte runtime struct. It likely contains:
- Persistent player profile data (name, stats, saved money)
- First byte = "exists" flag (initialized to 1)
